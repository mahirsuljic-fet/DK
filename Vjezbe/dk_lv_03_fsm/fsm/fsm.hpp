...
#define CAF_FSM_EVAL_MISMATCH_EC                                               \
  if (mismatch_ec == fsm::pec::unexpected_character)                           \
    ps.code = ch != '\n' ? mismatch_ec : fsm::pec::unexpected_newline;         \
  else                                                                         \
    ps.code = mismatch_ec;                                                     \
  goto fsm_after_fin;
...
#define state(name)                                                            \
  ...
  for (;;) {                                                                   \
    ...
    ps.code = ch != '\n' ? fsm::pec::unexpected_character                      \
                         : fsm::pec::unexpected_newline;                       \
    goto fsm_after_fin;                                                        \
...
#define fin()                                                                  \
  ...
  fsm_unexpected_eof:                                                          \
  ps.code = fsm::pec::unexpected_eof;                                          \
  goto fsm_after_fin;                                                          \
  fsm_fin:                                                                     \
  ps.code = fsm::pec::success;                                                 \
  fsm_after_fin:                                                               \
  static_cast<void>(0)
...
#define CAF_TERM_STATE_IMPL1(name)                                             \
  ...
  for (;;) {                                                                   \
    /* jumps back up here if no transition matches */                          \
    ps.code = fsm::pec::trailing_character;                                    \
    goto fsm_after_fin;                                                        \
...
#define CAF_TRANSITION_IMPL2(target, whitelist)                                \
  if (::fsm::in_whitelist(whitelist, ch)) {                                    \
...
#define CAF_FSM_TRANSITION_IMPL2(fsm_call, target)                             \
  ps.next();                                                                   \
  fsm_call;                                                                    \
  if (ps.code > fsm::pec::trailing_character)                                  \
    goto fsm_after_fin;                                                        \
  ch = ps.current();                                                           \
  goto s_##target;
